...
namespace HTraceAO.Scripts.Passes.URP
{
	internal class MotionVectorsURP : ScriptableRenderPass
	{
		...
		private ProfilingSampler ObjectMVProfilingSampler = new ProfilingSampler(HNames.HTRACE_OBJECTS_MV_PASS_NAME);
...
#pragma warning restore CS0618
		...
		public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
		{
			...
			using (IRasterRenderGraphBuilder builder = renderGraph.AddRasterRenderPass<CameraMVPassData>(HNames.HTRACE_CAMERA_MV_PASS_NAME, out var passData, new ProfilingSampler(HNames.HTRACE_CAMERA_MV_PASS_NAME)))
			{
				UniversalResourceData  resourceData  = frameData.Get<UniversalResourceData>();
