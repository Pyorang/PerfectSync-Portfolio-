...
public class ApplyAllIn1ShaderToPrefabs : EditorWindow
{
                        ...
                        for (int i = 0; i < materials.Length; i++)
                        {
                            if (materials[i] == null)
                            {
                                ...
                                else
                                {
                                    ...
                                    // Always bind renderer to the returned on-disk asset,
                                    // never the in-memory `newMat` (orphan safety).
                                    materials[i] = SaveMaterialAsset(newMat, prefab.name, childPath, i);
                                    newCount++;
                                }
                                rChanged = true;
                                continue;
                            ...
                            if (isAlreadyAllIn1)
                            {
                                ...
                                {
                                    // Orphan AllIn1 material — rebind renderer to the
                                    // returned on-disk asset to avoid hot-pink on save.
                                    materials[i] = SaveMaterialAsset(materials[i], prefab.name, childPath, i);
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                {
                                    // SaveMaterialAsset returns the on-disk asset.
                                    // For new files this is `target` itself, but capture
                                    // it explicitly to stay safe against future races.
                                    materials[i] = SaveMaterialAsset(target, prefab.name, childPath, i);
                                    newCount++;
                                }
    ...
    // after this call can leave prefab renderers pointing at a GUID-less orphan
    // Material, which serializes into the .prefab file as a missing reference
    // (hot-pink) on clean checkouts and other machines.
    static Material SaveMaterialAsset(Material mat, string prefabName, string childPath, int index)
    {
        ...
        if (existing != null)
        {
            ...
            // Clean up the orphan in-memory duplicate to prevent accidental reuse.
        ...
        // Force immediate import so GUID is assigned before we hand out a reference.
